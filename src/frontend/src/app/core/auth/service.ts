...
import { AntiforgeryResponse, AuthenticatedUser, LoginRequest } from "./models"
...
export class AuthService {
    ...
    login(request: LoginRequest) : Observable<AuthenticatedUser> {
        return this.refreshAntiforgeryToken().pipe(
            ...
            switchMap((user) => 
            this.refreshAntiforgeryToken().pipe(map(() => user),
        ),
    )
    ...
    private refreshAntiforgeryToken() : Observable<string> {
        return this.http.get<AntiforgeryResponse>(
            `${this.apiUrl}/security/antiforgery`, {
